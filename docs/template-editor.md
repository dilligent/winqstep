...
- DFT fields: basis file, potential file, XC functional, optional PBE
  ...
  PDOS, electron-density cube, Hartree-potential cube, and band-structure DFT
  print controls
...
by the renderer and workflow layer. The top of the tab includes a
`CP2K INPUT manual` hyperlink to the trunk CP2K input manual for keyword
details. Related controls are grouped by their CP2K input-section path with
...
`FORCE_EVAL/DFT/PRINT/PDOS`, `FORCE_EVAL/DFT/PRINT/E_DENSITY_CUBE`,
`FORCE_EVAL/DFT/PRINT/V_HARTREE_CUBE`, and
`FORCE_EVAL/DFT/PRINT/BAND_STRUCTURE` for
