...
class ReleaseBuildTests(unittest.TestCase):
    ...
    def test_release_plan_includes_runtime_files(self) -> None:
        ...
        self.assertIn("scripts/run_existing_input_batch.py", plan["files"])
        self.assertIn("scripts/manage_existing_input_batch.py", plan["files"])
        self.assertIn("scripts/start_gui.ps1", plan["files"])
        self.assertIn("scripts/gui/WinQStep.GuiControls.ps1", plan["files"])
    ...
    def test_release_builder_writes_zip_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn(f"{root}/scripts/run_existing_input_batch.py", names)
            self.assertIn(f"{root}/scripts/manage_existing_input_batch.py", names)
            self.assertIn(f"{root}/scripts/start_gui.ps1", names)
            self.assertIn(f"{root}/scripts/gui/WinQStep.GuiControls.ps1", names)
